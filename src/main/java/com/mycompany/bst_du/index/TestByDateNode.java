...
public final class TestByDateNode extends EntityNode<TestByDateNode> {
    ...
    public TestByDateNode(int ymd, long code, PcrTest ref){ this.ymd=ymd; this.code=code; this.ref=ref; }
    @Override public int compareTo(TestByDateNode o){
        int c = Integer.compare(this.ymd, o.ymd);
        if (c!=0) return c;
        return Long.compare(this.code, o.code);
    }
    @Override public String pretty(){ return "GDate{" + ymd + "," + code + "}"; }
}